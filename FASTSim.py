...
def get_veh(vnum):
    with open('..//docs//FASTSim_py_veh_db.csv','r') as csvfile: #automatically closes the file

        reader = csv.reader(csvfile) #create the reader file object
        vd = dict()
        data = dict()
        z=0
        #puts each row into data dictionary
        for i in reader:
           data[z]=i
           z=z+1
        ...
        if vnum in vd: #check if vehicle number in dictionary
            for i in range(len(variables)): #iterate through the variables
                vd[vnum][i]=str(vd[vnum][i]) #convert each value to string
                if vd[vnum][i].find('%') != -1: #ignore
...
def sim_drive_sub( cyc , veh , initSoc):
    ...
    kWhPerKg = 32.72
    kWhPerGGE = 33.7
    kWhPerGalDiesel = 37.95
    metersPerMile = 1609.00
    inchesPerMeter = 39.37
    gasolinekWhPerKg = 13.1
    dieselkWhPerKg = 12.61
    maxEpaAdj = .30
    litersPerGal = 3.7854
    maxFcEffKw =
